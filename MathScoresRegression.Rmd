...
Based on the scatterplot, we can see how most of the data falls either above or beneath the overall trend of the relationship between the FARMS percentage and the math test scores signficance. Based on this relationship between FARMS percentage and math test scores, there is a weak-positive relationship between the two variables.
...
This model is not worth using because the p value and r square value is not statistically significant so there’s no reason to use it if it’s not statistically significant. Without it being statistically signifcant, there is no way to prove a relationship which means what couldn't find what were trying to find and couldn't find a relationship between the two.
...
The biggest negative residuals is about 35 points below the predicted score at Whitemarsh Elementary School and the biggest positive residual then a 31 point above the predicted score at Crooked Billet Elementary School. This shows who is performing better than expected (Crooked Billet Elementary) and whose performing worse than expected (Whitemarsh Elementary School).
...
Based on the results of the models, it is not statisically signficant to say that students with reduced price meals do better on standardized math tests. With the models, you cannot say there is a relationship between the two because we don't have enough signficant data to prove it. This would influence a story pitch about the school board member and math stories because we wouldn't have the evidence and facts to back up this claim so we wouldn't be able to agree with what the school board member said and would have to state that and decide a story we could do with that. The story could be about how the board member doesn't have enough evidence to make that claim and give the evidence we have seen in the story.
